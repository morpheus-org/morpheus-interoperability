...
void coo_check_same(coo* A, coo* B){
  const ccl_index_t nnnz = morpheus_ccl_nnnz_mat_coo(A);
...
void csr_check_same(csr* A, csr* B){
  const ccl_index_t nrows = morpheus_ccl_nrows_mat_csr(A);
  const ccl_index_t nnnz = morpheus_ccl_nnnz_mat_csr(A);
...
void dia_check_same(dia* A, dia* B){
  ...
  ccl_mat_dense *Avals = morpheus_ccl_values_dia(A);
  ccl_mat_dense *Bvals = morpheus_ccl_values_dia(B);

  ccl_index_t Avals_rows = morpheus_ccl_nrows_mat_dense(Avals);
  ccl_index_t Avals_ncols = morpheus_ccl_ncols_mat_dense(Avals);
  ccl_index_t ndiags = morpheus_ccl_ndiags_mat_dia(A);
...
int main() {
  ...
  { 
    ...
    
    {
      ...
      csr *Acsr;
      dia *Adia;
      morpheus_ccl_create_default_mat_coo(&Acoo);
      morpheus_ccl_create_default_mat_csr(&Acsr);
      morpheus_ccl_create_default_mat_dia(&Adia);
      
      // csr->csr
      morpheus_ccl_convert_mat_csr_to_mat_csr_serial(refcsr, Acsr);
      csr_check_same(refcsr,Acsr);

      // csr->coo, coo->dia
      morpheus_ccl_convert_mat_csr_to_mat_coo_serial(refcsr, Acoo);
      coo_check_same(refcoo, Acoo);

      morpheus_ccl_convert_mat_coo_to_mat_dia_serial(Acoo, Adia);
      dia_check_same(refdia, Adia);

      // csr->dia
      morpheus_ccl_convert_mat_csr_to_mat_dia_serial(refcsr, Adia);
      dia_check_same(refdia, Adia);

      morpheus_ccl_destroy_mat_coo(&Acoo);
      morpheus_ccl_destroy_mat_csr(&Acsr);
...

    {
      ...
      // dia->dia
      morpheus_ccl_convert_mat_dia_to_mat_dia_serial(refdia, Adia);
      dia_check_same(refdia,Adia);

      // dia->coo, coo->csr
      morpheus_ccl_convert_mat_dia_to_mat_coo_serial(refdia, Acoo);
      coo_check_same(refcoo, Acoo);

      morpheus_ccl_convert_mat_coo_to_mat_csr_serial(Acoo, Acsr);
      csr_check_same(refcsr, Acsr);

      // dia->csr
      morpheus_ccl_convert_mat_dia_to_mat_csr_serial(refdia, Acsr);
      csr_check_same(refcsr, Acsr);
