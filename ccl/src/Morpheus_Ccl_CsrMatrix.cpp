...
void morpheus_ccl_create_mat_csr(ccl_mat_csr** A, ccl_index_t nrows,
  ...
  *A = (new ccl_mat_csr("ccl_mat_csr::", nrows, ncols, nnnz));
}

void morpheus_ccl_create_mat_csr_from_mat_csr(ccl_mat_csr* src,
                                              ccl_mat_csr** dst) {
  *dst = (new ccl_mat_csr(*src));
}
...
void morpheus_ccl_allocate_mat_csr_from_mat_csr(ccl_mat_csr* src,
                                                ccl_mat_csr* dst) {
  dst->allocate("ccl_mat_csr::allocate::", *src);
}

void morpheus_ccl_destroy_mat_csr(ccl_mat_csr** A) { delete (*A); }

ccl_index_t morpheus_ccl_nrows_mat_csr(ccl_mat_csr* A) { return A->nrows(); }

ccl_index_t morpheus_ccl_ncols_mat_csr(ccl_mat_csr* A) { return A->ncols(); }

ccl_index_t morpheus_ccl_nnnz_mat_csr(ccl_mat_csr* A) { return A->nnnz(); }
